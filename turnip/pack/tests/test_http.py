...
from turnip.pack import helpers, http
...
class FakeRoot:
    ...
    def authenticateWithPassword(self, user, password):
        ...
        return {
            "lp-int-data": 1,
            "lp-text-data": "banana",
            "lp-float-data": 1.23987,
            "lp-bool-data": True,
            "lp-none-data": None,
            "lp-bytes-data": b"bytes",
        }
...
class TestSmartHTTPRefsResource(ErrorTestMixin, TestCase):
    ...
    def test_backend_error(self):
        ...
        # to the path.
        yield self.performRequest(helpers.encode_packet(b"ERR so borked"))
        self.assertEqual(500, self.request.responseCode)
    ...
    def test_good_v2_included_version_and_capabilities(self):
        ...
        self.assertEqual(200, self.request.responseCode)
        self.assertEqual(
            self.root.client_factory.params,
            {
                b"version": b"2",
        ...
        self.assertEqual(
            capabilities + b"001e# service=git-upload-pack\n"
            b"0000001bI am git protocol data."
            b"And I am raw, since we got a good packet to start with.",
            self.request.value,
        )
...
class TestHTTPAuthLoginResource(TestCase):
    ...
    def test_render_GET_success(self):
        ...
        encoded_cookie = resource.signer.sign(
            encode_cookie(
                json.dumps(
                    {
    ...
    def test_getSession(self):
        ...
        resource = self.getResourceInstance(response)
        cookie_data = {"identity_url": "http://localhost", "user": "pappacena"}
