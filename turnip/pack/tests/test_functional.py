...
class FunctionalTestMixin(WithScenarios):
    ...
    def startPackBackend(self):
        ...
        self.backend_listener = reactor.listenTCP(
            ...
            PackBackendFactory(
                self.root,
                self.hookrpc_handler,
                self.hookrpc_sock_path,
                self.statsd_client,
            ),
        )
    ...
    def getProcessOutput(
        ...
        if executable == b"git":
            protocol_args = [
                b"-c",
                b"protocol.version=%s" % self.protocol_version,
            ]
    ...
    def getProcessOutputAndValue(
        self, executable, args=(), env=None, path=None, reactor=None
    ):
        if executable == b"git":
            protocol_args = [
                b"-c",
    ...
    def assertStatsdSuccess(self, repo, command):
        ...
        self.assertThat(
            ...
            MatchesDict(
                {
                    "git,operation={},repo={},env={},metric={}".format(
                        command,
                        repository,
                        config.get("statsd_environment"),
                        metric,
                    ): Not(Is(None))
    ...
    def test_statsd_metrics(self):
        ...
        yield self.assertCommandSuccess(
            (b"git", b"config", b"user.email", b"test@example.com"), path=clone
        )
        yield self.assertCommandSuccess(
    ...
    def test_no_repo(self):
        ...
        self.assertIn(
            b"Cloning into 'fail'...\n" + self.early_error + b"fatal: ", output
        )
    ...
    def test_no_permissions(self):
        ...
        self.assertIn(
            b"You do not have permission to create refs/heads/master.", error
        )
    ...
    def test_force_push(self):
        ...
        yield self.assertCommandSuccess(
            (b"git", b"reset", b"--soft", b"HEAD~2"), path=clone1
        )
        yield self.assertCommandSuccess(
        ...
        self.assertIn(b"You do not have permission to force-push to", error)
    ...
    def test_large_push(self):
        ...
        with open(os.path.join(clone1, "bigfile"), "wb") as bigfile:
            ...
            bigfile.write(
                bytearray(random.getrandbits(8) for _ in range(1024 * 1024))
            )
        yield self.assertCommandSuccess(
...
class FrontendFunctionalTestMixin(FunctionalTestMixin):
    ...
    def test_unicode_fault(self):
        def fake_translatePath(pathname, permission, auth_params):
            raise xmlrpc.Fault(2, "홍길동 is not a member of Project Team.")
    ...
    def make_set_symbolic_ref_request(self, line):
        ...
        url = urlunsplit(
            [
                ...
                parsed_url.path + b"/turnip-set-symbolic-ref",
                b"",
                b"",
            ]
        )
...
class TestSmartHTTPFrontendWithAuthFunctional(TestSmartHTTPFrontendFunctional):
    ...
    def setUp(self):
        ...
        self.url = (
            b"http://test-user:test-password@localhost:%d/+rw/test" % self.port
        )
        self.ro_url = (
...
class TestSmartSSHServiceFunctional(FrontendFunctionalTestMixin, TestCase):
    ...
    def setUp(self):
        ...
        self.service = SmartSSHService(
            ...
            self.virt_port,
            self.authserver_url,
            private_key_path=private_host_key,
            public_key_path=public_host_key,
            main_log="turnip",
            access_log="turnip.access",
