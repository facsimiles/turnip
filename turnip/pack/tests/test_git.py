...
from turnip.pack import git, helpers
...
class TestPackBackendProtocol(TestCase):
    ...
    def setUp(self):
        ...
        self.git_helper = os.path.join(
            os.path.dirname(git.__file__), "git_helper.py"
        ).encode("UTF-8")
    ...
    def setupConfig(self):
        ...
        # the wrong typing).
        config.defaults["virtinfo_timeout"] = "15"
    ...
    def test_create_repo_fails_to_confirm(self):
        ...
        def init_repo_side_effect(path, *args, **kwargs):
            os.mkdir(path)

        store.init_repo.side_effect = init_repo_side_effect
        ...
        self.assertEqual(
            [("foo.git",)], self.virtinfo.abort_repo_creation_call_args
        )
        self.assertEqual(
...
class TestPackVirtServerProtocol(TestCase):
    ...
    def test_translatePath(self):
        ...
        self.assertEqual(hashlib.sha256(b"/example").hexdigest(), pathname)
